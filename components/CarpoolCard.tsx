...
const CarIcon = () => (
    <svg xmlns="http://www.w.org/2000/svg" className="h-5 w-5 mr-2 text-gray-500" viewBox="0 0 20 20" fill="currentColor">
...
const CarpoolCard: React.FC<CarpoolCardProps> = ({ carpool, onBookClick, onCancelBooking, bookings, currentUser, onStartChat }) => {
    
    const getBookingForDay = (day: string): BookingRequest | null => {
      ...
      <div className="p-6">
        ...
        <div className="space-y-3">
          {carpool.schedule.map(s => {
            const booking = getBookingForDay(s.day);
            const status = booking?.status;
            const hasSeats = s.availableSeats > 0;
            ...
            const renderButton = () => {
                ...
                return (
                  <>
                    <button
                      ...
                      className="px-4 py-2 text-sm font-semibold rounded-md transition-all duration-200 bg-purple-500 text-white hover:bg-purple-600"
                    >
                      Chat
                    </button>
                    <button
                      onClick={() => onCancelBooking(booking!.id)}
                      className="px-4 py-2 text-sm font-semibold rounded-md transition-all duration-200 bg-red-500 text-white hover:bg-red-600"
                    >
                      Cancel
                    </button>
                  </>
              ...
              if (status === 'pending') {
                return (
                   <button
                        onClick={() => onCancelBooking(booking!.id)}
                        className="px-4 py-2 text-sm font-semibold rounded-md transition-all duration-200 bg-yellow-500 text-white hover:bg-yellow-600"
                    >
                        Cancel Request
                    </button>
                );
              ...
              if (hasSeats) {
                 return (
                    <button
                        ...
                        className="px-4 py-2 text-sm font-semibold rounded-md transition-all duration-200 bg-blue-600 text-white hover:bg-blue-700 focus:ring-blue-500"
                    >
                        Book a Seat
                    </button>
                 );
              ...
              return (
                 <button
                    disabled
                    className="px-4 py-2 text-sm font-semibold rounded-md bg-gray-300 text-gray-500 cursor-not-allowed"
                 >
                    Full
                 </button>
              );
            ...
            return (
                ...
                <div className="flex items-center space-x-4">
                    <span className={`text-sm font-bold ${hasSeats ? 'text-green-600' : 'text-red-600'}`}>
